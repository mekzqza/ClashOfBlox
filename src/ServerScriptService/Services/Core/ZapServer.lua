...
reliable.OnServerEvent:Connect(function(player, buff, inst)
	...
	while incoming_read < len do
		...
		if id == 5 then -- UpdateBuildingPosition
			local value
			value = {  }
		...
		elseif id == 1 then -- PlayerRquestRaid
			local value
		...
		elseif id == 3 then -- PlayerClickCollector
			local value
			value = {  }
		...
		elseif id == 4 then -- ConstructionComplete
			local value
			value = {  }
...
local returns = {
	...
	AssingZoneOwner = {
		...
		FireAll = function(Value: ({
			...
			alloc(1)
			buffer.writeu8(outgoing_buff, outgoing_apos, 3)
			local len_25 = #Value["FolderName"]
			assert(utf8.len(Value["FolderName"]) ~= nil, "value is not valid utf-8")
			alloc(2)
			buffer.writeu16(outgoing_buff, outgoing_apos, len_25)
			alloc(len_25)
			buffer.writestring(outgoing_buff, outgoing_apos, Value["FolderName"], len_25)
			local buff, used, inst = outgoing_buff, outgoing_used, outgoing_inst
			for _, player in Players:GetPlayers() do
				load_player(player)
				alloc(used)
		...
		FireExcept = function(Except: Player, Value: ({
			...
			alloc(1)
			buffer.writeu8(outgoing_buff, outgoing_apos, 3)
			local len_26 = #Value["FolderName"]
			assert(utf8.len(Value["FolderName"]) ~= nil, "value is not valid utf-8")
			alloc(2)
			buffer.writeu16(outgoing_buff, outgoing_apos, len_26)
			alloc(len_26)
			buffer.writestring(outgoing_buff, outgoing_apos, Value["FolderName"], len_26)
			local buff, used, inst = outgoing_buff, outgoing_used, outgoing_inst
			for _, player in Players:GetPlayers() do
				if player ~= Except then
					load_player(player)
